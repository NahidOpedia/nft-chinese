...
import { marketAbi, nftAbi, erc20Abi, factoryAbi } from '../src/components/utils/abi'
...
function App() {
  ...
  const setDefaultAddress = (address)=> {
   ...
   const setNftContract =(web3) => {
     setWallet({nftContract: new web3.eth.Contract(nftAbi, wallet.nftContractAddress)})
   }
   
   const setFactoryContract=  (web3)=> {
     setWallet({factoryContract: new web3.eth.Contract(factoryAbi, wallet.factoryContractAddress)})
   }
    const setMarketContract= (web3)=> {
     wallet.marketContract = new web3.eth.Contract(marketAbi, wallet.marketContractAddress);
   }
  ...
  .get(`${server}/api/user/info/`,{
    ...
    if (resu.data.code == 0) {
      window.localStorage.setItem('userInfo', JSON.stringify(resu.data.data))
      localStorage.setItem('lang', resu.data.data.language == 0 ? 'zh' : 'en')
    }
  })
  }
